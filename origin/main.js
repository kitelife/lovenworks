...
$(document).ready(
		function() {
			...
			if (window.location.hash) {
				...
				var active_link = $('.menu-item a[rel=' + active_id + ']')
						.closest('li');
				...
				// update classes
				$('.menu-item').removeClass('active').removeClass('next')
			...
			$.each($("#illustration .work"), function(index) {
				...
				if (column_counter == 1) {
					$(this).appendTo(first_column);
					column_counter = 2;
				} else if (column_counter == 2) {
					$(this).appendTo(second_column);
					column_counter = 3;
				} else {
					$(this).appendTo(third_column);
					column_counter = 1;
				}
				if ($("#illustration .work").length == 1) {
					$('#illustration').append(first_column).append(
							second_column).append(third_column);
				}
			});
			...
			$.each($("#graphic-design .work"), function(index) {
				...
				if ($("#graphic-design .work").length == 1) {
					$('#graphic-design').append(first_column).append(
							second_column).append(third_column);
				}
			});
					...
					function(event) {
						if (!$(this).is(':animated')) {
							...
							$('#content .previous').css({
								...
								$(this).remove();
							}).clone().addClass('next').removeClass('previous')
							...
							$('#navigation .previous').animate(
									...
									easing,
									function() {
										$(this).addClass('next').removeClass(
												'previous').appendTo(
												'#navigation').css({
											'width' : '33.3%'
										}).fadeTo(duration, 1);
									});
							$('#navigation .active').addClass('previous')
									.removeClass('active');
							$('#navigation .next').addClass('active')
									.removeClass('next');
					...
					function(event) {
						if (!$(this).is(':animated')) {
								...
								'left' : '-73%'
									...
									function() {
										$(this).addClass('active').removeClass(
												'previous');
									}).find('.page-number').fadeIn(duration,
									'easeInOutCubic').find('.back-to-top')
									.fadeIn();
							...
							$('#content .next').animate({
								...
								$(this).remove();
							}).clone().addClass('previous').removeClass('next')
									.prependTo('#content').css({
										'left' : '-151%'
									}).animate({
										'left' : '-73%'
									}, duration);
							...
							$('#navigation .next').animate(
									...
									function() {
										$(this).addClass('previous')
												.removeClass('next').prependTo(
														'#navigation').css({
													'width' : '33.3%'
												}).fadeTo(duration, 1);
									});
							$('#navigation .active').addClass('next')
									.removeClass('active');
							$('#navigation .previous').addClass('active')
									.removeClass('previous');
							event.stopPropagation();
							return false;
